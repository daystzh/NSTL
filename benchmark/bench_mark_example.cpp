...
constexpr auto my_pow(const int val) { return val * val; }
...
static void bench_array_at(benchmark::State &state) {
  ...
  for (auto _ : state) {
    arr.at(0) = my_pow(val);
    arr.at(1) = my_pow(val + 1);
    arr.at(2) = my_pow(val + 2);
    arr.at(3) = my_pow(val + 3);
    arr.at(4) = my_pow(val + 4);
    arr.at(5) = my_pow(val + 5);
  }
}
